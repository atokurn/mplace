...
import { ChevronDown, GripVertical, HelpCircle, ImageIcon, Trash2, Plus } from "lucide-react";
...
interface VariantsEditorProps {
  ...
  onReorderVariantOptions: (variantId: string, newOptions: VariantOption[]) => void;
}
    ...
    <>
      ...
      {addVariant && variants.map((variant) => (
        <div key={variant.id} className="space-y-4 p-4 border rounded-md bg-muted/20 relative">
          ...
          {/* Variant Options (Draggable) */}
          <Sortable
            value={variant.options}
            onValueChange={(items) => onReorderVariantOptions(variant.id, items)}
            getItemValue={(opt) => opt.id}
          >
            <SortableContent asChild>
              <div className="space-y-2">
                {variant.options.map((option, optionIndex) => (
                  <SortableItem value={option.id} asChild key={option.id}>
                    <div className="space-y-2">
                      {optionIndex === 0 && (
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <SortableItemHandle asChild>
                                  <Button variant="ghost" size="icon" className="h-9 w-9 text-muted-foreground cursor-grab">
          ...
          {/* End of variant options section */}
        </div>
      ))}
      ...
      {addVariant && (
        <div className="space-y-4 mt-4">
          ...
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold flex items-center"><span className="text-red-500 mr-1">*</span>List Varian</h3>
            <DropdownMenu>
